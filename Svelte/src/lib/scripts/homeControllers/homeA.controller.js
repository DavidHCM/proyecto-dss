...
export async function fetchDrivers() {
    try {
        ...
        const driverListContainer = document.querySelector('.driver-list-container');
        if (data.length > 0) {
...
export async function fetchRankings() {
    try {
        ...
        const userMap = new Map(rankings.users.map(user => [user.userId, user.name]));



        const rankingsHtml = rankings.rankings.map((ranking, index) => {
