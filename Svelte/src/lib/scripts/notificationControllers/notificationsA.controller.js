...
export async function createNotificationModal() {
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            const userId = type === "specificUser" ? document.getElementById("userId").value : null;
            const roleId = type === "specificRole" ? document.getElementById("roleId").value : null;
            ...
            return { message, type, userId, roleId, status: "sent", createdAt: new Date() };
        },
        didOpen: () => {
            ...
            typeSelect.addEventListener("change", () => {
                userDropdown.style.display = typeSelect.value === "specificUser" ? "block" : "none";
                roleDropdown.style.display = typeSelect.value === "specificRole" ? "block" : "none";
            });
        },
...
export async function fetchNotificationsUser() {
    try {
        ...
        const response = await fetch(`${apiUrl}/notification/byUser?userId=${userId}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
        ...
        if (notificationB && notificationB.length > 0) {
            const notificationsHtml = notificationB.map(notification => `
                <li class="notification-item">
                    <div class="notification-header">
                        <span><strong>ID:</strong> ${notification._id || 'N/A'}</span>
                        <span class="status ${notification.status}">${notification.status}</span>
