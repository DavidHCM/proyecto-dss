...
export async function fetchUsers() {
    try {
        ...
        if (data.length > 0) {
            const driverItems = data.map(user => `
...
export async function addUsers() {
    ...
    if (userInfo) {
        if (!userInfo.role) {
            Swal.fire("Error", "Por favor selecciona un rol para el usuario.", "error");
            return;
        }
        ...
        if (response.ok) {
            Swal.fire("Éxito", "El usuario se ha registrado correctamente.", "success");
            await fetchUsers();
        } else {
