...
export async function addDelivery() {
    ...
    const driverOptions = drivers.map(driver => `<option value="${driver.userId}">${driver.name}</option>`).join('');
    ...
    const { value: deliveryInfo } = await Swal.fire({
        ...
        didOpen: () => {
            ...
            const autocompleteDelivery = new google.maps.places.Autocomplete(deliveryInput);
...
export async function editDelivery(deliveryId) {
    ...
    const drivers = await fetchDrivers();
    const driverOptions = drivers.map(driver => `<option value="${driver.userId}" ${driver.userId === delivery.assignedTo ? 'selected' : ''}>${driver.name}</option>`).join('');
    ...
    const { value: deliveryInfo } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            return {
                pickupLocation: document.getElementById("swal-pickup").value,
                deliveryLocation: document.getElementById("swal-delivery").value,
                scheduledTime: document.getElementById("swal-fecha").value || scheduledDate,
                assignedTo: document.getElementById("swal-assign").value,
...
export async function fetchAllDeliveries() {
    ...
    if (response.ok) {
        ...
        if (deliveries.length > 0) {
            deliveryContainer.innerHTML = deliveries.map(delivery => `
                <li class="delivery-item">
                    <div class="delivery-header">
                        <span><strong>ID:</strong> ${delivery.deliveryId}</span>
                        <span class="status ${delivery.status.toLowerCase()}">${delivery.status}</span>
