...
export async function fetchIncidentsOpen() {
    try {
        ...
        const incidentContainer = document.querySelector(".incident-list-container");
        ...
        const userMap = new Map(responseData.user.map(user => [user.userId, user.name]));
...
export async function handleDeleteIncident(incidentId) {
    ...
    if (confirmResult.isConfirmed) {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Error al eliminar el incidente: ${response.statusText}`);
            }


            Swal.fire('Eliminado', 'El incidente ha sido eliminado correctamente.', 'success');
        } catch (error) {
            console.log(error)
            console.log(error.message)
            Swal.fire('Error', `No se pudo eliminar el incidente. ${error.message}`, 'error');
        }
    }
}
...
export async function handleEditIncident(incidentId) {
    ...
    if (formValues) {
        try {
            ...
            Swal.fire('Editado', 'El incidente ha sido actualizado correctamente.', 'success');
        } catch (error) {
            Swal.fire('Error', `No se pudo editar el incidente. ${error.message}`, 'error');
        }
    }
}
