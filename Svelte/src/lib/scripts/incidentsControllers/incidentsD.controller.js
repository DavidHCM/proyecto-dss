...
export async function addIncident() {
    ...
    const response = await fetch(`${apiUrl}/deliveries/byDriver?driverId=${userId}`, {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
        Swal.fire("Error", "No se pudieron obtener las entregas del usuario.", "error");
        return;
    }
    ...
    if (!Array.isArray(data.delivery)) {
        ...
        Swal.fire("Error", "El formato de los datos de entregas es incorrecto.", "error");
        return;
    }
    ...
    if (incidentInfo) {
        ...
        if (response.ok) {
            Swal.fire("Éxito", "El incidente ha sido reportado correctamente.", "success");
            await fetchByDriver();
        } else {
...
export async function fetchByDriver() {
    try {
        ...
        const response = await fetch(`${apiUrl}/incident/byDriver?driverId=${userId}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`,
        ...
        if (incidents.length > 0) {
            const userMap = new Map([[responseData.user.userId, responseData.user.name]]);
